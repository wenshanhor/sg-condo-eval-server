...
export class UraClient {
   ...
   * and filter by project name. Includes sub-sale (2) and resale (3).
   */
    ...
    for (const batch of batches) {
      ...
      for (const project of batch.Result) {
        ...
        for (const tx of project.transaction || []) {
          ...
          if (tx.typeOfSale !== "2" && tx.typeOfSale !== "3") continue;
          allResale.push(tx);
  ...
  /**
   * Find comparable sub-sale & resale transactions from nearby developments.
